"""CRM connector sample input + normalized + mapping coherence tests."""
...
def test_malformed_sample_raises_readable_error(connector_helpers):
    ...
    assert broken["lead_interaction"], "test requires at least one interaction sample"
    broken["lead_interaction"][0].pop("interaction_id", None)
    with pytest.raises(ValueError) as exc:
        connector_helpers["assert_sample_conforms"](broken, schema, "lead_interaction")
