"""Manual Uploads connector sample input + normalized + mapping coherence tests."""
...
def test_malformed_sample_raises_readable_error(connector_helpers):
    ...
    assert broken["budget_file"], "test requires at least one budget_file sample"
    broken["budget_file"][0].pop("property_id", None)
    with pytest.raises(ValueError) as exc:
        connector_helpers["assert_sample_conforms"](broken, schema, "budget_file")
    assert "property_id" in str(exc.value)
