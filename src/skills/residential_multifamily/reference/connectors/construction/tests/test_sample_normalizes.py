"""Construction connector sample input + normalized + mapping coherence tests."""
...
def test_malformed_sample_raises_readable_error(connector_helpers):
    ...
    assert broken["change_order"], "test requires at least one change_order sample"
    broken["change_order"][0].pop("change_order_id", None)
    with pytest.raises(ValueError) as exc:
        connector_helpers["assert_sample_conforms"](broken, schema, "change_order")
    assert "change_order_id" in str(exc.value)
