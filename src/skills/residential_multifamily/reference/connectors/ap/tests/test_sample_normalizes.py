"""AP connector sample input + normalized + mapping coherence tests."""
...
def test_malformed_sample_raises_readable_error(connector_helpers):
    ...
    assert broken["invoice"], "test requires at least one invoice sample"
    broken["invoice"][0].pop("invoice_id", None)
    with pytest.raises(ValueError) as exc:
        connector_helpers["assert_sample_conforms"](broken, schema, "invoice")
