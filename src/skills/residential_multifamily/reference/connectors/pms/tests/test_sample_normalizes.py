"""PMS connector sample input + normalized + mapping coherence tests."""
...
def test_malformed_sample_raises_readable_error(connector_helpers):
    """Negative case: strip primary key and confirm the helper surfaces it."""
    schema = connector_helpers["load_yaml"](CONNECTOR_DIR / "schema.yaml")
    normalized = connector_helpers["load_json"](CONNECTOR_DIR / "sample_normalized.json")
    broken = copy.deepcopy(normalized)
    assert broken["lease"], "test requires at least one lease sample"
    broken["lease"][0].pop("lease_id", None)
    with pytest.raises(ValueError) as exc:
        connector_helpers["assert_sample_conforms"](broken, schema, "lease")
