"""GL connector sample input + normalized + mapping coherence tests."""
...
def test_malformed_sample_raises_readable_error(connector_helpers):
    ...
    assert broken["actual"], "test requires at least one actual sample"
    broken["actual"][0].pop("account_id", None)
    with pytest.raises(ValueError) as exc:
        connector_helpers["assert_sample_conforms"](broken, schema, "actual")
