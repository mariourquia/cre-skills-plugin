...
- On failure (network error, non-2xx): confirm "Feedback saved locally. Remote submission failed -- it will be retried next session." Append the submission to `~/.cre-skills/outbox.jsonl` via `hooks/feedback-outbox.mjs` `enqueue()` for automatic retry on next session start.
