...
Download [`cre-skills-plugin-v4.4.0.dmg`](https://github.com/mariourquia/cre-skills-plugin/releases/latest) from the latest release.
...
Download [`cre-skills-plugin-v4.4.0-setup.exe`](https://github.com/mariourquia/cre-skills-plugin/releases/latest) from the latest release.
...
Restart Claude Desktop, then start a new conversation. The plugin's MCP server gives Claude its tools it can call automatically. Core tools:

| Tool | What it does | Example prompt |
|------|-------------|----------------|
| `cre_route` | Routes your request to the right CRE skill | "Screen this deal -- 240-unit multifamily, Raleigh, $42M" |
| `cre_list_skills` | Browse and filter all skills | "What leasing skills do you have?" |
...
The plugin itself is a structured knowledge base of process documents, reference files, and expert agent definitions. Claude reads these docs to guide its analysis -- no external services or AI models beyond Claude itself.
...
|---------|---------|------|
| Skills available | All skills (via MCP routing) | All skills (via slash commands) |
...
Both platforms access the same skill library. Desktop users just interact through natural conversation instead of slash commands.
