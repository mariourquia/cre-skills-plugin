...
export function buildTarget(target: TargetName, quiet = false): boolean {
  ...
  // Copy dist/catalog.json if it exists (generated by catalog-build.py)
  const srcCatalogJson = resolve(REPO_ROOT, "dist", "catalog.json");
  if (existsSync(srcCatalogJson)) {
    mkdirSync(resolve(outDir, "dist"), { recursive: true });
    cpSync(srcCatalogJson, resolve(outDir, "dist", "catalog.json"));
    log(`  dist/catalog:  copied`);
  }

  // Copy essential scripts (redact-feedback.mjs needed by send-feedback/report-problem commands)
  const essentialScripts = ["redact-feedback.mjs"];
  const scriptsOutDir = resolve(outDir, "scripts");
  for (const script of essentialScripts) {
    const srcScript = resolve(REPO_ROOT, "scripts", script);
    if (existsSync(srcScript)) {
      mkdirSync(scriptsOutDir, { recursive: true });
      cpSync(srcScript, resolve(scriptsOutDir, script));
    }
  }
  log(`  scripts:       essential scripts copied`);
